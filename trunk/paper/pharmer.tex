...
\begin{itemize}
	\item \emph{Search and retrieval:} more efficient and effective search interfaces, such as faceted search~\cite{tunkenlang2009faceted} or question answering~\cite{Lopez2011}.
	...
	\item \emph{Information presentation:} more sophisticated ways of flexibly visualizing information, such as by means of semantic overlays as described in~\cite{Burel2009}.
	\item \emph{Information integration:} unified views on heterogeneous data stored in different applications by creating composite applications such as semantic mashups~\cite{Ankolekar2007}.
	\item \emph{Personalization:} customized and context-specific information which better fits user needs and will
result in delivering customized applications such as personalized semantic portals~\cite{ecs2007}.
	\item \emph{Reusability} and \emph{interoperability}: exchanging content between disparate systems and enabling applications such as executable papers~\cite{Muller2011}.
\end{itemize}
...
The Pharmer implementation is open-source and available for download together with an explanatory video and online demo at \cite{pharmerdemo,pharmerproject}.
...
To pursue this, it utilizes datasets such as DrugBank, DailyMed and RxNorm (available at \cite{lodds}) by sending federated SPARQL~\cite{sparql} queries.
...
Annotator adopts the RDFa format. \emph{RDFa}~\cite{rdfa} (Resource Description Framework in attributes) is a W3C Recommendation that adds a set of attribute level extensions to XHTML for embedding RDF metadata within web documents.
...
\begin{figure*}[tb]
	\centering
		\includegraphics[width=1.8\columnwidth]{images/screenshot1.jpg}
	\caption{Screenshot of the Pharmer information view and prescription writer.}
